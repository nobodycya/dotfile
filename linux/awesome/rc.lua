...
beautiful.init({
  ...
  menu_height = dpi(40),
  menu_width = dpi(200),
  taglist_squares_sel = theme_assets.taglist_squares_sel(dpi(0), "#000000"),
  taglist_squares_unsel = theme_assets.taglist_squares_unsel(dpi(0), "#000000"),
  ...
  awesome_icon = theme_assets.awesome_icon(dpi(15), "#555555", "#ffffff"),
...
awful.screen.connect_for_each_screen(function(s)
  ...
  s.wibox:setup({
    ...
    -- Right
    {
      ...
      wibox.widget({
        awful.widget.watch(
        [[sh -c "echo \  $(top -bn1 | grep Cpu | awk '{print sprintf(\"\%.0f\", $2)}')%\ "]],
        3
        ),
      ...
      wibox.widget({
        awful.widget.watch(
        [[sh -c "echo \  $(free | grep Mem | awk '{printf \"%.0f\", ($3/$2)*100}')%\ "]],
        3
        ),
        widget = wibox.container.background,
        fg = "#eeeeee",
      ...
      wibox.widget({
        awful.widget.watch([[sh -c "echo \ 󱈏 $(cat /sys/class/power_supply/BAT1/capacity)%\ "]], 120),
        widget = wibox.container.background,
        fg = "#eeeeee",
      }),
      wibox.widget({
        awful.widget.watch(
        [[sh -c "echo \ 󰕾 $(echo $(amixer sget Master | grep -o -E '[0-9]+%' | head -1))\ "]],
        1
        ),
        widget = wibox.container.background,
        fg = "#eeeeee",
      ...
      wibox.widget({
        awful.widget.watch([[sh -c "echo \ 󰖙 $(($(brightnessctl get) * 100 / $(brightnessctl max)))%\ "]], 1),
        widget = wibox.container.background,
        fg = "#eeeeee",
      }),
      wibox.widget({
        awful.widget.watch([[sh -c "echo \ 󰃰 $(date +%Y-%m-%d\ %H:%M)\ "]], 60),
        widget = wibox.container.background,
        fg = "#eeeeee",
      }),
...
awful.rules.rules = {
  -- All clients will match this rule.
  {
    ...
    properties = {
      ...
      awful.key({ modkey, "Ctrl", "Shift" }, "Tab", function(c)
        c:move_to_screen()
      end)
      ),
